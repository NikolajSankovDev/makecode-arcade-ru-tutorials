...
- :redo: Из ``||loops:Циклы||`` перетащите
```block
pause(1000)
```
его в **конец** <br/>
...
- :mouse pointer: Измените текст на тот, который будет служить первой строкой вашей истории. Вы также можете изменить расположение текста с ``||game(noclick):внизу||`` на другую область экрана.
...
- :paper plane: Из ``||sprites:Спрайты||``, возьмите
```block
...
- :mouse pointer: Выберите свой спрайт, затем просмотрите категорию ``||sprites:Спрайты||``, чтобы найти другие блоки, которые
